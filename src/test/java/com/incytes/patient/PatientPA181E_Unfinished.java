package com.incytes.patient;

import com.codeborne.selenide.Condition;
import com.codeborne.selenide.Selenide;
import org.testng.annotations.AfterMethod;
import org.testng.annotations.Test;

public class PatientPA181E_Unfinished {

    @Test
    public void c_test() {
        Main main = new Main();
        Main.Registration reg = main.new Registration();
        Main.FileTXT file = main.new FileTXT("D:\\Path\\count.txt");
        int count = Integer.parseInt(file.getText());
        reg.open("https://alpha-patient-enrollment.incytesdata-dev.com?data=");
        reg.setAll("qwertyuiop17091709+" + count + "@yandex.by", Main.password, Main.password, "", "", "", "Belarus", "").wRegistration().clickLogin();
        file.writeText(String.valueOf(count+1), false);
        reg.wwRegistration();
        reg.eCountryName().shouldHave(Condition.text("Belarus"));
    }
    @AfterMethod
    public static void close(){
        Selenide.close();
    }
}