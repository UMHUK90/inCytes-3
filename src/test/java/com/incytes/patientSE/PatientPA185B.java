...
public class PatientPA185B {
    ...
    @Test
    public void c_test() {
        System.setProperty("chromeoptions.prefs","intl.accept_languages=es");
        open(baseUrl + "auth/login");
        EnterEmail();
        EnterPassword();
        clickSignInButton();
        $$("p").findBy(exactText("ACERCA DE GRAPH")).shouldBe(visible);
        $$("p").findBy(text("SP - Scoring Group Test #6 Description - Single Questions Only.")).shouldBe(visible);
        ...
        $$("h4").findBy(exactText("Lip injection SP")).shouldBe(visible);
        $$("p").findBy(exactText("Lip injection description SP")).shouldBe(visible);
        ...
        $$("h4").findBy(exactText("Knee Pain in Spanish")).shouldBe(visible);
        $$("p").findBy(exactText("Knee Pain in Spanish")).shouldBe(visible);
        close();
    }
}
