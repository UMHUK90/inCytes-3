...
public class PatientPA185D {
    ...
    @Test
    public void c_test() {
        System.setProperty("chromeoptions.prefs","intl.accept_languages=it");
        open(baseUrl + "auth/login");
        EnterEmail();
        EnterPassword();
        clickSignInButton();
        $$("p").findBy(exactText("INFORMAZIONI SU GRAPH")).shouldBe(visible);
        $$("p").findBy(text("IT - Scoring Group Test #6 Description - Single Questions Only.")).shouldBe(visible);
        ...
        $$("h4").findBy(exactText("Lip injection IT")).shouldBe(visible);
        $$("p").findBy(exactText("Lip injection description IT")).shouldBe(visible);
        ...
        $$("h4").findBy(exactText("Knee Pain in Italian")).shouldBe(visible);
        $$("p").findBy(exactText("Knee Pain in Italian")).shouldBe(visible);
        close();
    }
}
